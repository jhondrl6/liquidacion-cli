...
- **S17 — Tarea 1.E (ParamsProvider year-aware) — CERRADA**: 3 archivos creados/modificados (275 insertions, 16 deletions). (1) `liquidator/core/params_provider.py` (177 lineas) — `ParamsProvider` con `for_year()`/`for_date()`/`for_range()`/`current()`/`reload()`, accesores `SMMLV`/`AUXILIO_TRANS`/`TASA_INT_CESANTIAS`/`DIAS_BASE`/`TOPE_INDEMNIZACION_SMMLV`, `params_version`, `to_dict()`, singleton thread-safe. (2) `liquidator/core/__init__.py` — exporta `ParamsProvider`. (3) `liquidator/cli/main.py` — usa `ParamsProvider.for_year()`, actualiza mensajes de estado. **Tests:** `test_params_provider.py` — 9 tests. **9/9 PASS** en 0.51s. Suite completa: **270 pass / 45 fail / 25 errors** — 0 regresiones (+9 pass vs S16 baseline 261/45/25). CLI verificada funcional. Desbloquea Tarea 1.D (JSONGenerator refactor).
- **S18 — Tarea 1.D (JSONGenerator refactor con ParamsProvider + schema opcional) — CERRADA, commit `402627b`**: 3 archivos modificados (~734 insertions, ~277 deletions). (1) `liquidator/output/json_generator.py` reescrito (~370 líneas) con nuevo contrato del plan §6.2: `__init__(self, schema_path=None, params=None)`; si `params` no se inyecta → `ParamsProvider.current().to_dict()` (elimina constantes hardcodeadas SMMLV=1423500/AUXILIO_TRANS=200000/LIMITE_AUXILIO=2847000/TASA_INT_CESANTIAS=0.12/DIAS_BASE=360/TOPE_INDEMNIZACION_SMMLV=20/FECHA_APLICACION_RECARGO_DOMINICAL="2025-07-01" del refactor previo); método principal `generate_output(calculation_result: dict)` que retorna shape compatible con Pydantic `LiquidacionResult` (Tarea 1.C) — campos `meta`/`trabajador`/`empleador`/`parametros`/`contrato`/`desglose`/`total_liquidacion`/`validaciones_y_alertas`/`normas_aplicadas`/`compliance_report`; `meta` con 11 campos (`modo` normalizado a MAYÚSCULAS / `fecha_generacion` / `motor_version`=__version__ / `generator_version`=__version__ / `params_version` del params inyectado / `params_hash` SHA-256 del subset canónico de params / `input_hash` SHA-256 del input / `output_hash` SHA-256 del output / `parametros_por_segmento` / `compliance_status` / `referencias_normativas` / `plantilla_version=None`); validación contra JSON Schema opcional (si `schema_path` apunta a archivo existente → `jsonschema.validate`; si no existe → no falla); extractores tolerantes a múltiples shapes (input_data o top-level, calculation_results o top-level, compliance_report o compliance); shim deprecated `generate_json(input, calc, comp, params)` que arma el dict unificado y delega a `generate_output` (compat con engine y Fase 0); alias `save_json` de `save_to_file`. (2) `liquidator/core/engine.py` migrado a `generate_output(calculation_result)` con dict unificado (input_data + calculation_results + compliance_report + validaciones_y_alertas + normas_aplicadas); prefija `self.json_generator.params = params` para que el generador no re-lea disco; **elimina los overrides post-generador** (`output["validaciones_y_alertas"] = alerts; output["normas_aplicadas"] = workflow_result.normas_aplicadas`) que ya no son necesarios porque van dentro del cálculo. (3) `liquidator/tests/test_output/test_json_generator.py` **escrito de nuevo** (17 tests, 612 insertions, 277 deletions) — cubre el contrato del plan §6.2: `test_json_generator_usa_params_inyectados` y `test_json_generator_valida_contra_schema_si_existe` (los 2 sugeridos), más 15 adicionales (default constructor con ParamsProvider, schema_path inexistente sin falla, schema_path existente preservado, shape compatible con Pydantic, `meta` con 11 campos, `parametros` refleja inyección, `params_version` viene de params inyectado/ParamsProvider, `modo` normalizado a mayúsculas, `validaciones_y_alertas`/`normas_aplicadas` faltantes no fallan, `total_liquidacion` viene del calc, validación contra schema exitoso, validación contra schema falla si no cumple, hash calculation estable, `save_to_file` escribe JSON válido, `save_json` retorna True, shim `generate_json` delega a `generate_output`). **17/17 PASS** en 0.58s. Suite completa: **287 pass / 41 fail / 25 errors** — 0 regresiones (+17 pass / -4 fail vs S17 baseline 270/45/25; el delta -4 fail son los 4 tests viejos del JSONGenerator que ahora son los 17 nuevos). **Bug pre-existente de Fase 0 confirmado en `test_engine.py::test_engine_process_input_generates_output`** (mismo error `TypeError: unsupported operand type(s) for /: 'str' and 'str'` en `checklist_loader.py:21` con `git stash` y re-run → no es regresión de 1.D, queda en R-OP-02). **DoD (plan §6.2 1.D):** 5/5 puntos cumplidos (constructor con schema_path+params, default a ParamsProvider.current().to_dict(), constantes hardcodeadas eliminadas, validaciones_y_alertas consistente, validación contra schema opcional). **2 validaciones del plan (líneas 1497-1508):** ambas passing (`test_json_generator_usa_params_inyectados`, `test_json_generator_valida_contra_schema_si_existe`). **Desbloquea:** Tarea 1.I (test caso canónico end-to-end — bloqueada por 1.D según REGISTRY).
...
- **Fase 0 CERRADA en S11 (con DoD 4/5 = 80% verificado en S11.6).** Siguiente fase: **Fase 1** (1.A-utils S12, 1.X S13, 1.A-plan S14, 1.B S15, 1.C S16, **1.E S17**, **1.D S18**). Próxima: **Tarea 1.G** (R-OP-03, fix schema 1 min, decisión S11.6) o **Tarea 1.I** (test caso canónico end-to-end, desbloqueada por 1.D) o **Tarea 1.C-bis/ter/quater** (addendas, anidadas en Fase 1).
