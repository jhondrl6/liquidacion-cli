...
- **Próxima tarea a ejecutar:** **Tarea 1.A-plan** (`pyproject.toml` y packaging — plan §6.2 Tarea 1.A original, NO confundir con la Tarea 1.A de utils cerrada en S12). Esta tarea es prerequisito de 1.B (CLI) porque el entry point necesita `pip install -e .` para resolver imports de paquete. **Alternativa:** si el usuario prefiere avance comercial rápido, atacar 1.A-plan + 1.B en la misma sesión. **Recordatorios para Fase 1:** (i) **Tarea 1.G lleva R-OP-03** (schema fix, 1 min, diferido de S11.6); (ii) **Tarea 1.E (ParamsProvider year-aware)** sigue siendo prerequisito de cualquier ejecución real con segmentos 2025/2026; (iii) **R-OP-02 Causa 2** (9 fails en test_versioning.py por `datetime.now` regression) sigue pendiente.
