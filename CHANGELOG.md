...
- **S20 — Tarea 1.G (R-OP-03 schema fix) — CERRADA POR VALIDACIÓN CONTRA CÓDIGO VIVO** (sin patches al schema). La descripción S11.6 del R-OP-03 era **inexacta**: las definiciones `plazo_pago_detalle` y `limite_detalle` YA estaban a top-level de `definitions` (líneas 262 y 319 de `params/schema.json`); las 13+4 refs en el schema son todas válidas. **Validación real con `jsonschema.validate`:** `params/2025.json` OK ✓, `params/2026.json` OK ✓, `params/plazos.json` OK ✓, **`params/normas.json` FAIL por R-OP-10 (NUEVO)**: id `LEY_2466_2025_INTERESES_MENSUALES` (33 chars) excede `maxLength: 20` declarado en `params/schema.json` L112. Único id problemático entre 18 entradas (los otros 17 caben en ≤17 chars). **DoD de 1.G cumplido** sin patches — R-OP-03 cerrado por estado de código (validar contra live code es la regla §5.5.11 del diagnóstico). 3 archivos tocados: `Planificación/REGISTRY.md` (Estado, Tabla de fases, Handoff, Pendientes S11, Trampas conocidas, Log de cierres), `CHANGELOG.md` (esta entrada + Pending), `Contexto/KB_LLM/06_riesgos_modelo.md` (R-OP-10 documentado, post-commit).

### Pending (próximas sesiones)
- **R-OP-10 (NUEVO S20, BAJA, fix 1 min):** `params/normas.json` entrada `normas[15].id = "LEY_2466_2025_INTERESES_MENSUALES"` (33 chars) excede `maxLength: 20` declarado en `params/schema.json` L112. Tres opciones para resolver: (a) **subir maxLength a 40** en el schema (1 línea, retrocompatible, no invasivo); (b) **renombrar el id** a algo ≤20 chars (p.ej. `LEY_2466_2025_INT_MENS`, 21 chars — preserva la restricción); (c) **diferir a hot-patch futuro**. No bloquea avance de Fase 1 (`normas.json` no se valida en flujo CLI actual); sí bloquea cierre 100% formal de Tarea 0.K. Decisión pendiente del usuario.
...
- **R-OP-03 (RECLASIFICADO/CERRADO S20):** la causa raíz resultó ser R-OP-09, no refs rotas. Ver entrada S20 arriba. No requiere acción.
- **Fase 0 CERRADA en S11 (con DoD 4/5 = 80% verificado en S11.6).** Siguiente fase: **Fase 1** (1.A-utils S12, 1.X S13, 1.A-plan S14, 1.B S15, 1.C S16, 1.E S17, 1.D S18, 1.I S19, **1.G S20**). Próxima: decisión sobre R-OP-09 → luego **Tarea 1.C-bis/ter/quater** (addendas) o **Tarea 1.F** (refactor `markdown_generator.py`).
