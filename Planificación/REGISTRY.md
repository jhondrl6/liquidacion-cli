...
- **Última sesión cerrada:** **S37 — 2026-06-14, Tarea 4.A (promoción a v2.0.0)**. `pyproject.toml` version=2.0.0, `liquidator/__init__.py` __version__=2.0.0, `CHANGELOG.md` con entrada v2.0.0 (breaking changes + added + detailed changes), `README.md` actualizado (título, CLI, versión, fecha). `liquidacion --version` → `2.0.0`. Development Status: 5 - Production/Stable. 0 cambios de código, 0 regresiones a tests.
- **Próxima tarea a ejecutar:** **Fase 4 — Tarea 4.B** (CI mínima: `.github/workflows/ci.yml` o `scripts/ci.sh`). Plan §9.2: compileall + ruff + pytest + KB freshness. Pendientes: 4.B CI, 4.C README rewrite, 4.D limpieza documental, 4.E tag, **4.F 3 liquidaciones reales verificadas por Jhond** (DoD bloqueante manual).
...
| 4          | v2.0 release                                          | **EN CURSO**  | —       | **4.A ✓ (S37):** `pyproject.toml` version=2.0.0, `__init__.py` __version__=2.0.0, `CHANGELOG.md` v2.0.0, `README.md` actualizado, `liquidacion --version`=2.0.0. Pendiente: 4.B CI, 4.C README rewrite, 4.D limpieza, 4.E tag, 4.F 3 liquidaciones reales. |
