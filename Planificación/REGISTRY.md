...
- **Última sesión cerrada:** **S49 — 2026-06-19, Tarea 4.F (paso 3, ejecución — 3 liquidaciones reales verificadas)**. Sesión de validación end-to-end: 3 casos provistos por el usuario (`Planificación/Casos.md` borradores), validados contra código vivo del motor v2.0. **3 discrepancias mayores conceptuales identificadas en el manual del usuario (NO bugs del motor):** (1) Caso 1 prima — manual asumió período completo, motor aplica Art. 306-308 CST semestral (153 días H2 2024 → $706.350, manual $1.246.500). (2) Caso 1 vacaciones — manual asumió $562.500 en PERIODICA, motor $0 per Art. 189 CST (compensación solo con acuerdo mutuo explícito). (3) Caso 3 cobertura período — manual solo fracción 2025 (180d ficticios), motor 912 días completos con cap 360. **Discrepancias menores aceptadas:** Caso 1 diff días (275 vs 270, +1.85% en cesantías); Caso 3 factor IPC hipotético 1.1098 vs DANE real 1.0754 (−$120.367). **Usuario confirmó acuerdo** con análisis y conclusiones: el motor aplica correctamente Art. 249-250 CST (cap 360 año completo), Art. 306-308 CST (prima semestral SBL fecha_corte), Art. 189 CST (vacaciones en PERIODICA = tiempo no dinero), Art. 488 CST (prescripción 3 años), Ley 50/1990 Art. 99 (intereses 12%), SL2630-2024 (anualización por año calendario), Art. 46 CST (preaviso 30 días). **Decisión per STRATEGY §6 punto 3:** "Cálculo manual obsoleto → actualizar cálculo manual, no es bug del motor". **6 archivos en `audit/validacion_v2/`:**
...
- **Próxima tarea a ejecutar:** **Fase 4 — cleanup post-4.F para cierre formal**. Tarea 4.F cerrada. Fase 4 sigue **EN CURSO** con 3 reparos documentados pendientes: (1) **Item 11 DoD §9.1 "Suite al 100%"** — pytest crónicamente rojo (36F+15E preexistentes, S45 cronología). Triagear 36F+15E por categoría (features a medio implementar / tests deprecados / regresiones no detectadas) y abrir tarea dedicada de cleanup. (2) **Verbatim SL2630-2024** (reparo bloqueante (b) del Addendum SL2630-2024 + IPC) — entrada `SL2630_2024` en `params/normas.json` L180-186 marcada `PENDIENTE_VERBATIM` desde S28. Verificar texto literal, sala y URL oficial en SUIN. (3) **Verbatim Art. 488 CST** (reparo bloqueante análogo) — entrada `CST_488_PRESCRIPCION` en `params/normas.json` L191-195 marcada `PENDIENTE_VERBATIM`. Verificar texto literal en SUIN. Cierre formal de Fase 4 requiere los 3 items cerrados + suite verde.
- **Bloqueos activos:** ninguno de Fase 0 (cerrada). PENDIENTE (pospuesto por usuario en S3): eliminación de `__pycache__/`, `htmlcov/`, `.coverage`, `liquidacion_nomina_colombia.egg-info/`, `documentos_legales_rurales/` (vacío). **Issue resuelto post-S26:** `.env.backup*` del .gitignore ahora SÍ matchea — `.gitignore` reescrito sin inline comments (R-OP-09 cerrado). **Issue post-S10:** `pytest` con `uv run --with pytest …` SÍ funciona en WSL. **Issue post-S11:** `python3` directo denegado por sandbox — mitigación: `uv run --with <pkg> python3 <script>`. **Issue legal post-S11:** nulidad del Decreto 1469/2025 pendiente (R-LEG-07) — SMMLV 2026 vigente por D. 159/2026; vigilar antes de v2.0 release. **CI crónicamente rojo en pytest (S45, documentado):** GitHub Actions run #1, #2, #3, #5, #6 (S38, S39, S40, S44×2) fallan en step 8 (Tests/pytest) por 36 fails + 15 errors preexistentes de Fase 2+ (gaps heredados). Run #4 (S43) falló en step 6 (Lint/ruff) por side-effect del refactor de .gitignore — resuelto en S44. Estado post-S44: ruff verde, mypy verde (con `|| true`), KB freshness verde, **pytest crónicamente rojo**. Item 11 del DoD Fase 4 ("Suite al 100%") sigue abierto. No bloquea 4.F (paso 3) pero bloquea cierre formal de Fase 4. Acción futura: triagear los 36F+15E por categoría (¿features a medio implementar, tests deprecados, regresiones no detectadas?) y abrir tarea dedicada en cleanup post-release. **Verbatim SL2630-2024 pendiente (S46, documentado):** entrada `SL2630_2024` en `params/normas.json` (L180-186) marcada `PENDIENTE_VERBATIM`. Falta verificar texto literal, sala y URL oficial de SL2630-2024 en SUIN antes de cerrar v2.0 release. Reparo bloqueante (b) del Addendum SL2630-2024 + IPC. **Verbatim Art. 488 CST pendiente (S46, documentado):** entrada `CST_488_PRESCRIPCION` en `params/normas.json` (L191-195) marcada `PENDIENTE_VERBATIM`. Falta verificar texto literal del Art. 488 CST en SUIN antes de cerrar v2.0 release. Modelo usa Art. 488 (NO Art. 155, error común) — el motor está correcto pero la verificación documental del texto literal está pendiente. Reparo bloqueante (b) del Addendum. **Node 20 actions deprecadas en CI (S46, documentado):** `.github/workflows/ci.yml` usa `actions/checkout@v4` y `actions/setup-python@v5` (ambos Node 20). GitHub forzará Node 24 desde 16-Jun-2026 (es hoy) y removerá Node 20 el 16-Sep-2026. Cambio mecánico: bumpear a `@v5` y `@v6` (o equivalente) o setear `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` como workaround temporal. Warning deprecation aparece en todos los runs.
...
|||| 4          | v2.0 release                                          | **EN CURSO**  | —       | **4.A ✓ (S37), 4.B ✓ (S38), 4.C ✓ (S39), 4.D ✓ (S40), 4.E ✓ (S41), 4.F ✓ S49 (ejecución 3 liquidaciones validadas)**. CI con compileall + ruff + pytest + kb_freshness + `scripts/ci.sh`. ruff 0 errores. README.md reescrito v2.0 (8/8 secciones plan §9.2). Limpieza documental: QWEN.md → CONTRIBUTING.md, stale JSONs eliminados, code_quality_analysis.md reescrito. Tag v2.0.0 creado y pusheado a origin. **4.F cerrada S49:** 3 liquidaciones reales verificadas (Básico PERIODICA + Avanzado FINIQUITO SBL variable + Complejo FINIQUITO IPC), motor v2.0 correcto per normatividad sustantiva y jurisprudencial. 3 discrepancias mayores en manual del usuario documentadas como "manual obsoleto, motor es la verdad" per STRATEGY §6. Pendientes para cierre formal Fase 4: (a) Item 11 "Suite al 100%" (36F+15E crónicos, S45), (b) verbatim SL2630-2024, (c) verbatim Art. 488 CST. |
