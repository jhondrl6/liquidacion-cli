...
- **S23 — Tarea 1.C-bis (Schema `Salario` extendido, addendum SL2630-2024) — CERRADA**. Aditiva retrocompatible sobre la base 1.C (Tarea 1.C, S16). 3 archivos modificados: (1) `liquidator/contracts/input_model.py` — agregado sub-modelo `MesValor` (`año: int`, `mes: int` 1-12, `valor: Decimal` >0); extendida `Salario` con `sbl_por_anio: dict[int, Decimal] | None = None` + `historial_salarial: list[MesValor] | None = None`; agregado `model_validator(mode="after")` que rechaza `variable=True` sin historial (mensaje claro "Salario variable requiere historial_salarial o sbl_por_anio"); `__all__` exporta `MesValor`; imports extendidos con `model_validator`. (2) `liquidator/tests/test_contracts/test_salario_extendido.py` (nuevo, 10 tests): regresión canónica (`test_salario_sin_campos_nuevos_es_compatible`, `test_salario_solo_sbl_variable_false_pasa`), validación consistencia (`test_salario_variable_sin_historial_falla`, `test_salario_variable_sin_historial_pero_con_sbl_por_anio_pasa`, `test_salario_variable_con_historial_pasa`), forma B.1.a (`test_salario_con_sbl_por_anio_pasa`), validación `MesValor` (`test_mesvalor_valida_rango_mes`, `test_mesvalor_valida_valor_positivo`, `test_mesvalor_rango_mes_bordes_pasa`), integración con `LiquidacionInput` (`test_liquidacion_input_acepta_salario_extendido`). (3) `Contexto/KB_LLM/04_schema_entrada.md` — sección "Schema Pydantic formal" actualizada (campos 1.C-bis marcados IMPLEMENTADO S23); lista de campos pendientes para 1.C-bis corregida (eliminado `intereses_mensuales_acuerdo` que NO está en el addendum SL2630; agregado `historial_salarial` y `model_validator` que SÍ están). **Validación DoD (plan §6.2 1.C-bis):** 10/10 PASS en `test_salario_extendido.py`; 39/39 PASS en `test_contracts/` (29 previos + 10 nuevos); 11/11 PASS en `test_caso_canonico.py` (regresión canónica). **No regresión:** 0 nuevos failures; baseline S22 325/38/25 → S23 335/38/17 (+10 pass, 0 fail delta, -8 errors por reorganización de `--ignore`). R3 del plan mitigado: 0 inputs en `examples/inputs/` con `variable=true` (grep confirmado). Decisión de patch directo — el motor NO consume los nuevos campos en Fase 1; Tarea 2.B-bis (Fase 2) los activará vía `SalarioResolver` (addendum §B.2). Reparos bloqueantes del addendum: (a) Art. 488 CST vs Art. 155 NO tocado en esta tarea; (b) SL2630-2024 verbatim NO tocado (queda para Tarea 2.X); (c) IPC como índices acumulados NO tocado (queda para Tarea 2.X).
