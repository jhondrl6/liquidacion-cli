...
- **Última sesión cerrada:** **S43 — 2026-06-15, Tarea 4.F (en curso, cont.)**. Dead-block V014 corregido: hooks `_calcular_vacaciones_si_finiquito` y `_calcular_preaviso_si_fijo_vencido` movidos a pre-compliance (engine.py L111-138). Regresión introducida en S33 (Tarea 2.Z agregó V014 CRITICAL que evaluaba el desglose ANTES de que el hook generara el renglón → NO_GO → hook nunca corría). Golden `test_finiquito_renuncia_212d.py` 5/5 PASS (antes 0/5); suite completa 656P/36F/1xfail/15E (antes 651P/41F/1xfail/15E); +5 PASS, 0 regresiones. Tests de Tarea 3.D (rama NO_GO) intactos (20/20 PASS). V014 ahora PASS con evidencia "Vacaciones compensadas en finiquito: 550,000 COP (7.5 días, Art. 189-190 CST)".
- **Próxima tarea a ejecutar:** **Fase 4 — Tarea 4.F** (paso 3). Una vez cerrado el fix del dead-block (S43), el paso 3 es DoD bloqueante manual: vos proveés 3 liquidaciones reales → correr CLI con `audit/validacion_v2/caso_N/input.json` → llenar `comparativa.md` con cálculo manual vs. motor → cierre de 4.F y de Fase 4.
