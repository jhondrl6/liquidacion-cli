...
- **Última sesión cerrada:** S12 — 2026-06-13 (Tarea 1.A de Fase 1 cerrada: 10 símbolos utils desbloqueados en `liquidator/utils/__init__.py` — 6 funciones nuevas implementadas + 4 exports de currency agregados + 2 exports de error_handler. Collection errors: 8 → 3. DoD: 5/8 collection errors cerrados + 10 utils symbols + 0 regresiones + 31 asserts unitarios PASS. 2 hallazgos nuevos: R-OP-05 (3 collection errors restantes en `liquidator/params/`) y R-OP-06 (3 runtime failures en `test_utils/` por signatures incompatibles). Convención "1 tarea por sesión" respetada — la siguiente sesión arranca con la próxima tarea de Fase 1).
- **Última fase cerrada:** Fase 0 (Higiene + segundo cerebro mínimo) — 0.A-0.K ✓. Siguiente fase: Fase 1 (Estabilizar y formalizar).
- **Próxima tarea a ejecutar:** **Tarea 1.B** del plan §6.2 (`liquidator/cli/main.py` — entry point real basado en Click) **o** una sub-tarea 1.B-extra para limpiar los 3 collection errors de R-OP-05 (params exports) y los 3 runtime failures de R-OP-06 (signatures de `add_business_days`/`business_days_between` con `holidays` kwarg + redefinir `parse_date`/`days_between_inclusive` como funciones reales). **Decisión del usuario en próxima sesión** sobre cuál atacar primero. **Recordatorios para Fase 1:** (i) **Tarea 1.G lleva R-OP-03** (schema fix, 1 min, decisión del usuario en S11.6 de diferir); (ii) **Tarea 1.E (ParamsProvider year-aware)** sigue siendo prerequisito de cualquier ejecución real con segmentos 2025/2026.
...
\| 1          | Estabilizar y formalizar                              | EN CURSO (Tarea 1.A cerrada S12) | — | Tarea 1.A cerrada en S12 (10 símbolos utils desbloqueados, 5/8 collection errors cerrados — ver log S12). 3 collection errors restantes → R-OP-05 (asignable a 1.B o 1.X). 3 runtime failures en `test_utils/` → R-OP-06 (asignable a 1.B o 1.X). Plan §6.2 Tarea 1.B (`liquidator/cli/main.py`) y 1.E (ParamsProvider year-aware) son los próximos hitos lógicos. |
...
|----|--------------------|---------------------------------------------------------------------------------|----------------------------------------|------------------|--------------------------------------------------------------------------------------------------------------------------------------|
| (vacío) | — | — | — | — | **Fase 0 CERRADA en S11 (2026-06-13).** Tareas 0.A-0.K todas ✓. Siguiente fase: Fase 1 Tarea 1.A (fix `SalaryError` export en `liquidator/utils/__init__.py` — 1 línea, Causa 1 de R-OP-02). |
...
- **R-OP-05 (NUEVO S12):** 3 collection errors restantes en
  `liquidator/params/` por exports faltantes.
  `liquidator/params/params_loader.py` no exporta `ParamsError` ni
  `ParamsSource`; `liquidator/params/params_validator.py` no exporta
  `ValidationError`. Tests bloqueados: `test_params/test_loader.py`,
  `test_params/test_params_loader.py`, `test_params/test_validator.py`.
  Fix trivial (~3 líneas); asignable a Tarea 1.B o tarea 1.X. NO
  bloquea 1.A (Causa 1 de R-OP-02 ya está 5/8).
- **R-OP-06 (NUEVO S12):** 3 runtime failures en
  `test_utils/test_date_currency_utils.py` por signatures
  incompatibles. Causa: alias `parse_date = calculate_days_between`
  y `days_between_inclusive = calculate_days_between` en
  `liquidator/utils/__init__.py:18-19` apuntan a funciones str-based
  que NO matchean el contrato date-based esperado por los tests;
  `add_business_days` no soporta kwarg `holidays`. NO bloquea 1.A
  (4 de 7 tests PASS ya); pero impide validar la suite completa.
  Asignar a 1.B o 1.X.
