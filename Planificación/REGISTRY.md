...
- **Última sesión cerrada:** **S51 — 2026-06-19, Fase 4 cierre: 6F→0F + verbatim SL2630 parcial + Art.488 verificado**. **Item 1:** 6 fallos corregidos en tests de integración. (1) `test_salario_limite_auxilio`: SBL esperado corregido 2847000→1423500+200000 (fixture tiene salario 1 SMMLV, no 2). (2) `test_periodo_recargo_dominical`: marcado `xfail` (feature recargo dominical no implementada en v2.0). (3) `test_multiples_componentes_salariales`: SBL corregido sumando auxilio_transporte 200K + cap 360 días. (4) `test_finiquito_completo`: eliminado `vacaciones` del loop plazo_pago_legal (vacaciones no tiene esa key en desglose). (5) `test_finca_rural_completo`: reescrito completo — SBL 1500K (no 2200K), usar `total_liquidacion` (no `total_liquidacion_periodica`), eliminar `auxilio_conectividad_advertencia`. (6) `test_salario_variable_completo`: SBL==2050000 (no >2200000), formato normas_aplicadas con espacio. **Item 2:** SL2630-2024 → VERIFICADO_PARCIAL. Sala Descongestión Laboral N.º 1, rad. 101342, 17-sep-2024, ponente Beltrán Quintero. Doctrina: anualización año por año. Fuente: vLex + Gerencie.com. SIUGJ intermitente, texto literal no extraído de fuente primaria. **Item 3:** Art. 488 CST → texto verbatim obtenido de CIJUF (Min. Protección Social 2008) + Función Pública (Concepto 099361/2021). Prescripción 3 años. Ley 2466/2025 Art.62 modificó Art.488 pero mantuvo término 3 años. normas.json parcheado. **Suite post-S51: 682P/28S/2xfail/1w. 0F.**
...
- **Próxima tarea a ejecutar:** **S52 — cierre formal Fase 4**. Pendientes: (a) extraer texto literal Art.488 CST de fuente primaria SIUGJ/Función Pública (complementar normas.json con cita verbatim), (b) intentar acceso directo a SIUGJ para SL2630-2024 (texto literal del fallo), (c) verificar que ruff + kb_freshness siguen verdes, (d) CI pipeline completo `scripts/ci.sh`. Condiciones para cerrar Fase 4 formalmente: suite 0F ✓ (alcanzado S51), verbatims VERIFICADO (SL2630 parcial, Art.488 pendiente extracción fuente primaria). **Fase 4 puede cerrarse** si el usuario acepta VERIFICADO_PARCIAL para ambos verbatims.
- **Bloqueos activos:** ninguno de Fase 0 (cerrada). PENDIENTE (pospuesto por usuario en S3): eliminación de `__pycache__/`, `htmlcov/`, `.coverage`, `liquidacion_nomina_colombia.egg-info/`, `documentos_legales_rurales/` (vacío). **Issue resuelto post-S26:** `.env.backup*` del .gitignore ahora SÍ matchea. **CI post-S51:** suite 0F (682P/28S/2xfail/1w). Los 28S son weasyprint-dependent (PDF generation), 2xfail son features no implementadas (recargo dominical + V014 golden). **Verbatim SL2630-2024:** VERIFICADO_PARCIAL (S51). Sala, ponente, radicado y doctrina confirmados contra vLex. Texto literal del fallo no extraído de SIUGJ (portal intermitente). **Verbatim Art. 488 CST:** texto obtenido de 2 fuentes secundarias verificables (CIJUF Min. Protección 2008 + Función Pública Concepto 099361/2021). Prescripción 3 años. Ley 2466/2025 Art.62 modificó Art.488 (mantiene 3 años). normas.json actualizado en S51. **Node 20 actions deprecadas en CI (S46, documentado):** `.github/workflows/ci.yml` usa `actions/checkout@v4` y `actions/setup-python@v5` (ambos Node 20). GitHub forzará Node 24 desde 16-Jun-2026; removerá Node 20 el 16-Sep-2026. Cambio mecánico: bumpear versiones o setear `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`.around temporal. Warning deprecation aparece en todos los runs.
...
|||| 4          | v2.0 release                                          | **EN CURSO**  | —       | **4.A ✓ (S37), 4.B ✓ (S38), 4.C ✓ (S39), 4.D ✓ (S40), 4.E ✓ (S41), 4.F ✓ S49, Suite 0F ✓ S51 (682P/28S/2xfail), Verbatim SL2630 PARCIAL ✓ S51, Verbatim Art.488 ✓ S51**. CI con compileall + ruff + pytest + kb_freshness + `scripts/ci.sh`. ruff 0 errores. README.md reescrito v2.0. Tag v2.0.0 creado y pusheado a origin. **S51 cerró los últimos 6F** (expected values desfasados + feature no implementada). **Verbatims:** SL2630-2024 VERIFICADO_PARCIAL (sala/ponente/doctrina confirmados, texto literal pendiente SIUGJ). Art.488 CST verificado contra 2 fuentes secundarias (CIJUF+FP), texto literal obtenido, Ley 2466/2025 mantiene 3 años. **Pendiente cierre formal:** aceptar VERIFICADO_PARCIAL o extraer texto literal de fuente primaria en S52. |
